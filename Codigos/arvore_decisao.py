...
def decision_tree_parametro(df):
...
def decision_tree_sem_parametro(df):
     # Separar as características e o rótulo
    x = df.drop('perigo', axis=1)
    y = df['perigo']
...
def decision_tree_parametro_accuracy(df):
    # Separar as características e o rótulo
    x = df.drop('perigo', axis=1)
    y = df['perigo']
...
def decision_tree_parametro_sem(df):
...
def decision_tree_sem_parametro_sem(df):
    #separa em x as variaveis e em y se isso faz ser perigoso ou não
    x = df.drop('perigo', axis=1)
    y = df['perigo']
...
def decision_tree_parametro_accuracy_sem(df):
     #separa em x as variaveis e em y se isso faz ser perigoso ou não
    x = df.drop('perigo', axis=1)
    y = df['perigo']
...
def main():
    ...
    #com tratamento
    #best_clf, grid = decision_tree_parametro(df)
    #print(best_clf)
    #decision_tree_sem_parametro(df)
    #best_clf, grid = decision_tree_parametro_accuracy(df)
    #print(best_clf)

    #sem tratamento
    best_clf, grid = decision_tree_parametro_sem(df)
    print(best_clf)
    decision_tree_sem_parametro_sem(df)
    best_clf, grid = decision_tree_parametro_accuracy_sem(df)
    print(best_clf)
